...
static void ispccdc_print_status(struct isp_ccdc_device *ccdc)
{
	...
	dev_dbg(isp->dev, "ccdc data format is %d\n",
		ccdc->formats[CCDC_PAD_SINK].code);
			      ...
			      struct ispccdc_update_config *ccdc_struct)
{
	...
	if (ISP_ABS_TBL_LSC & ccdc_struct->update) {
		...
		if (ISP_ABS_CCDC_CONFIG_LSC & ccdc_struct->update &&
		    ISP_ABS_CCDC_CONFIG_LSC & ccdc_struct->flag) {
			/* Using new config */
...
static void ccdc_configure(struct isp_ccdc_device *ccdc)
{
	...
	isp_select_bridge_input(isp, ccdc->input);
	...
	isp_reg_writel(isp, ((format->height - 2) << ISPCCDC_VDINT_0_SHIFT) |
		       (0 << ISPCCDC_VDINT_1_SHIFT), OMAP3_ISP_IOMEM_CCDC,
		       ISPCCDC_VDINT);
...
void ispccdc_hs_vs_isr(struct isp_ccdc_device *ccdc)
{
